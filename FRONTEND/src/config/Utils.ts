export const formatRelativeTime = (createdAtString:string) => {
  if (!createdAtString) return "";

  const videoDate = new Date(createdAtString );
  const currentDate = new Date();
  
  const diffInMs = currentDate.getTime() - videoDate.getTime();
...
export const dummyRecommendations:RecommendationType[] = [
  {
    _id: "rec1",
