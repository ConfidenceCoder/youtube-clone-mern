...
import { dummyRecommendations } from '../config/Utils' 
...
if (!currentVideo || !channel) {
    return <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center">Loading Video...</div>;
  }
  return (
    ...
    <div className="flex flex-col lg:flex-row gap-6 p-4 lg:p-6 bg-gray-900 min-h-screen text-white">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <img
              src={
                channel?.img ||
                `https://ui-avatars.com/api/?name=${ channel?.username || 'U'}&background=random&color=fff&size=128`
              }
              alt="channel-avatar"
            ...
            <div>
              <h2 className="font-semibold text-sm md:text-base leading-tight">{channel.username}</h2>
      ...
      ${currentUser?.subscribedUsers?.includes(channel._id)
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-2 md:pb-0 hide-scrollbar">
            ...
            <button className="flex items-center gap-2 bg-gray-800 px-4 py-2 rounded-full hover:bg-gray-700 transition-colors text-sm font-medium ">
              🔗 Share
            </button>
            <button className="flex items-center gap-2 bg-gray-800 px-4 py-2 rounded-full hover:bg-gray-700 transition-colors text-sm font-medium ">
              ➕ Save
        ...
        {dummyRecommendations.map((video) => (
          <div key={video._id} className="flex gap-2 cursor-pointer hover:bg-gray-800 p-2 rounded-lg transition-colors">
            <img
              src={video.imgUrl}
              alt="thumb"
              className="w-40 h-24 object-cover rounded-lg bg-gray-700 "
            />
