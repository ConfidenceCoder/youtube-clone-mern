...
export const signin = wrapAsync(async (req:Request, res:Response) => {
   ...
   const { password: userPassword, ...others } = existingUser.toObject();

    res.cookie("access_token", token, cookieOptions).status(200).json(others);
})

export const googleAuth = wrapAsync(async (req:Request, res:Response) => {
    ...
    if (existingUser) {
        ...
        const token = jwt.sign({ id: existingUser._id }, process.env.JWT_SECRET as string);
        const { password, ...others } = existingUser.toObject();
